...
ax1.errorbar(Nmidss_MR, Nx_val/Nx_count/10**14, xerr=Nerrors_x, yerr=Nerrors_y/10**14, fmt='.',color = 'blue', capsize=5, label='Non-physical')  #gives errors if divide by 0
ax1.errorbar(Pmidss_MR, Px_val/Px_count/10**14, xerr=Perrors_x, yerr=Perrors_y/10**14, fmt='.',color = 'red', capsize=5, label='Physical')  #gives errors if divide by 0

ax1.set_ylim(MRlim3,MRlim4)
ax2.set_ylim(0,0.75) 

ax2.errorbar(Pmidss_MR, excess/10**14, xerr=Perrors_x, yerr=excessER/10**14, fmt='.',color = 'black', capsize=5, label='Excess')  #gives errors if divide by 0
ax2.legend()
ax1.legend()
...
plt.savefig('M-MR.pdf', dpi=1200,bbox_inches='tight')