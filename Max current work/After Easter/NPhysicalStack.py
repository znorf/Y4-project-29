    ...
    for indices in ind:                                     #ind (index) is in a wierd format, this for loop turns a long list of numbers into a list where each new number is a new column.
        for i in indices:
            ...
            if np.abs(D1[0,2]-data.iloc[i,3])< zslice and i not in indVir1 and i not in indVir2:     #This filters the haloes so that only those within z values +- a certain value from the first cluster in the non-physical pair are allowed. The second statement filters out all haloes within 2*Virial radius of the first and second clusters in the non-physical pair
                ind1.append(i) 
    ind = tree.query_radius(D2, r=(np.sqrt((1.5*XYZsep)**2+ XYZsep**2+ zslice**2)))    #repeat the steps above for the second cluster in the non-physical pair.            
    for indices in ind:
        for i in indices:
...
def Nplot():                                                                                    #get non-physical pairs data for plot          
    ...
    plt.plot(xc1,yc1,'r.')
    plt.plot(xc2,yc2,'b.') 
    plt.xlim(D1[0,0]-0.5*XYZsep, D1[0,0]+1.5*XYZsep)  
    plt.ylim(D1[0,1]-XYZsep, D1[0,1]+XYZsep)                                                                           
    plt.plot(D1[0,0],D1[0,1],color = 'orange',marker='*',markersize=15)
...
i=0 
select = 200  #max 4608
rand = [random.randint(0, (len(Npairs)/2)-1) for _ in range(select)]  
rand = np.arange(0,400,2)                                                                 #A while loop to create density arrays for all non-physical paris in the non-physical pair catalogue 'pairs'.         
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#
