...
def load(select):
    ...
    return n0,n1,pid,n0Chunks,n1Chunks,dataH13,rand,Stack,bins,select,binss
...
def Dataset(Type):  
    ...
    return D1,D2,R1,R2,XYZsep,data,cluster1,cluster2,zslice,multiplier
...
def Nenvironment():             #A function to find the indexes of DM haloes around each of the two clusters        
    ...
    ind = tree.query_radius(D2, r=(np.sqrt((XYZsep)**2+ (XYZsep/2)**2+ zslice**2)))
    for indices in ind:
        for i in indices:
...
def Nrotation(rotate):                           #A function for non-physical pairs to create arrays containing x and y coordinates of DM haloes and their mass densities. This is done for the two clusters separately. Cluster 2 is rotated by a random value between 0 and 2 pi, given by the variable 'rotate'. 
    ...
    for i in ind2:                              #DM halo masses and co-ords for cluster 2. x and y co-ords are randomly rotated and then put at a distance of 1 * Xsep to the right of cluster 1.                     
        ...
        rot = rot + D1     #Translate the new x,y,z coordinates that are centred at cluster 1.
...
n0,n1,pid,n0Chunks,n1Chunks,dataH13,rand,Stack,bins,select,binss = load(100)
Nran = []
i=0
for i in rand:  
    if n0.iloc[i,0] == 'no_pair':
        ...
        D1,D2,R1,R2,XYZsep,data,cluster1,cluster2,zslice,multiplier = Dataset(0)
        if len(chunks) < 10 and XYZsep < 20:                                     #ignoring wierd chunk boundary plots and (currently) ignoring pairs that go over due to periodicity
            ...
            Nenvironment()                                                       
            R,mc1 = Nrotation(random.randint(0, 628)/100)  #get the x,y and mass density data                                                                                      
            Nplot()                     #create histograms 
            if n1.iloc[i,0] == 'no_pair':
                print('no pair')
            else:
                D1,D2,R1,R2,XYZsep,data,cluster1,cluster2,zslice,multiplier = Dataset(1)
