...
def load():
    ...
    #rand = np.array([59])
    return Ppairs1,Ppairs2,Ppairs11,Ppairs22,df_Chunk,rand,Stack,bins,select
...
def PValues(D1origin,D2origin):                                                                          #All of the physical cluster data for calculations 
    ...
    return zslice,vol,theta_z,theta_y,lims,XYZsep,multiplier,D1,D2  
...
Ppairs1,Ppairs2,Ppairs11,Ppairs22,df_Chunk,rand,Stack,bins,select = load()
...
for i in rand:  
    ...
    if XYZsep < 20:         #for now ignoring plots where cluster 1 and cluster 2 are on the opposite sides of the box due but are pairs due to periodicity. 
        zslice,vol,theta_z,theta_y,lims,XYZsep,multiplier,D1,D2 = PValues(D1origin,D2origin)    
        Penvironment()                                                       
        xc1,yc1,mc1 = Protation(theta_z,theta_y)  #get the x,y and mass density data                                                                                     
