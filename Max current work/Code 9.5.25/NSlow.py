...
def Nplot():                                                                                    #plotting sum of physical and non-physical pairs      
    ...
    StackMR.append(MR)
    FilHaloes.append(len(mcf))

 
    for i in range(len(binsSep)):
        if XYZsep < binsSep[i]:
            SepTotal[count3,i-1] = sum(mcf)           
            break
    for i in range(len(binsMR)):
        if MR < binsMR[i]:
            MRTotal[count3,i-1] = sum(mcf)
...
def Final():   
    ...
    np.savetxt("Nstats.csv",stats_data,delimiter=",",header=column_headers,comments="",fmt="%.4f")
...
print(end-start) 
n0,n1,pid,n0Chunks,n1Chunks,dataH13,rand,Stack,bins,StackRad,binss,Stackhaloes,StackSep,binsSep,binsMR,StackMR,StackValues,FilHaloes,SepTotal,MRTotal,select= load(100)
