...
def load(select):
    ...
    return n0,n1,pid,n0Chunks,n1Chunks,dataH13,rand,Stack,bins,select,StackRad,binss
...
def NValues(D1,D2):                                                                         
    ...
    volStack = length * height * zslice  *2                                                    #volume of each box in the histogram
    ...
    for i in ind1:                              
        ...
        if r > binss[0] and x < (20-(2*R2)*multiplier) and x > (0+(2*R1)*multiplier):
            ...
            for k in range(len(binss)):
                if r < binss[k]:
                    vol = (np.pi*(XYZsep-(2*R2+2*R1))*(binss[k]**2)-(np.pi*(XYZsep-(2*R2+2*R1))*(binss[k-1]**2)))   #pi r^2 l = cylinder volume
