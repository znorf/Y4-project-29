...
def load(select):
    ...
    binsSep = np.linspace(3,20,10)
    binsMR = np.linspace(0,1,10)
    FilMassSep = np.zeros(len(binsSep)-1)
    FilMassMR = np.zeros(len(binsMR)-1)
    FilMassSepTally = np.zeros(len(binsSep)-1)
    FilMassMRTally = np.zeros(len(binsMR)-1)
    ...
    return n0,n1,pid,n0Chunks,n1Chunks,dataH13,rand,Stack,bins,StackRad,binss,Stackhaloes,StackRhaloes,StackSep,binsSep,binsMR,FilMassSep,FilMassMR,FilMassMRTally,FilMassSepTally,StackMR,StackValues,StackFil
...
def Nrotation(rotate):                           #A function for physical pairs to create arrays containing x and y coordinates of DM haloes and their mass densities. This is done for the two clusters separately. Cluster 2 is rotated by a random value between 0 and 2 pi, given by the variable 'rotate'. 
    xc1,xc2,yc1,yc2,mc1,mc2,mcf = [],[],[],[],[],[],[]   #stacking code
    mcT,R = [],[]   #radius code
    global rot
    for i in ind1:                              
        ...
        if r > binss[0] and x < 20 and x > 0:
            ...
            for k in range(len(binss)):
                if r < binss[k]:
                    vol = (np.pi*(XYZsep)*(binss[k]**2)-(np.pi*(XYZsep)*(binss[k-1]**2)))   #pi r^2 l = cylinder volume
    ...
    R = R.tolist()    
    return xc1,xc2,yc1,yc2,mc1,mc2,R,mcT,mcf
...
def Nplot():                                                                                    #plotting sum of physical and non-physical pairs      
    ...
    StackRhaloes.append(len(mcT))
    StackSep.append(XYZsep)
    StackMR.append(MR)
    StackFil.append(len(mcf))
    #StackValues[0].append(len(mc1)+len(mc2))    #no. haloes in stacking histogram
    #StackValues[1].append(len(mcT))             #no. haloes in radius density plot
    #StackValues[2].append(XYZsep)               #list of XYZsep
    #StackValues[3].append(MR)                   #list of Mass ratios
 
    for i in range(len(binsSep)):
        if XYZsep < binsSep[i]:
            FilMassSep[i-1] += sum(mcf)
            FilMassSepTally[i-1] +=1
            break
    for i in range(len(binsMR)):
        if MR < binsMR[i]:
            FilMassMR[i-1] += sum(mcf)
            FilMassMRTally[i-1] +=1
            break

    #print(StackRhaloes,Stackhaloes,StackSep)
    ...
    '''
    return Stackhaloes,StackRhaloes,StackSep,StackMR,StackFil,FilMassMRTally,FilMassSepTally,FilMassMR,FilMassSep
def Final():
    ...
    for i in range(len(binsSep) - 1):
        ...
        midss.append(m)
    #errors_x = np.std(StackSep, ddof=1)/(np.sqrt(sum(FilMassSepTally)))
    #errors_y = np.std(mcf,ddof=1)/(np.sqrt(sum(FilMassMRTally)))
    plt.figure()
                                                              
    #plt.errorbar(midss, FilMassSep/FilMassSepTally, xerr=errors_x, yerr=errors_y, fmt='-o', capsize=5, label='Data ± Error')  #gives errors if divide by 0
    plt.plot(midss, FilMassSep/FilMassSepTally) 
    ...
    for i in range(len(binsMR) - 1):
        ...
        midss.append(m)
    #errors_y = np.std(mcf,ddof=1)/(np.sqrt(sum(FilMassMRTally)))
    #errors_x = np.std(StackMR, ddof=1)/(np.sqrt(sum(FilMassMRTally)))
    plt.figure()
    #plt.errorbar(midss, FilMassMR/FilMassMRTally,xerr=errors_x, yerr=errors_y, fmt='-o', capsize=5, label='Data ± Error') 
    plt.plot(midss, FilMassMR/FilMassMRTally) 

    plt.xlabel('Mass Ratio')
    plt.ylabel('Mass/solar masses')
    plt.xlim(0,1)
    print('mean number of haloes in stacking plot is' ,np.mean(Stackhaloes), 'standard deviation is ',  np.std(Stackhaloes, ddof=1))
    print('mean number of haloes in radius plot is',np.mean(StackRhaloes),'standard deviation is ',np.std(StackRhaloes, ddof=1))
    print('mean number of haloes in filament only is',np.mean(StackFil),'standard deviation is ',np.std(StackFil, ddof=1))
    print('mean separation is', np.mean(StackSep),'standard deviation is ',np.std(StackSep, ddof=1))
    print('mean mass of Filament is',np.nansum(FilMassSep)/(count*2) )
    print('number of plots is ', count*2)
...
print(end-start) 
n0,n1,pid,n0Chunks,n1Chunks,dataH13,rand,Stack,bins,StackRad,binss,Stackhaloes,StackRhaloes,StackSep,binsSep,binsMR,FilMassSep,FilMassMR,FilMassMRTally,FilMassSepTally,StackMR,StackValues,StackFil= load(20)
...
for i in rand:  
    ...
    if n0.iloc[i,0] == 'no_pair':
        ...
        if len(chunks) < 8 and XYZsep < 20:                                     #ignoring wierd chunk boundary plots and (currently) ignoring pairs that go over due to periodicity
            ...
            Nenvironment()                                                       
            xc1,xc2,yc1,yc2,mc1,mc2,R,mcT,mcf = Nrotation(random.randint(0, 628)/100)  #get the x,y and mass density data                                                                                      
            Stackhaloes,StackRhaloes,StackSep,StackMR,StackFil,FilMassMRTally,FilMassSepTally,FilMassMR,FilMassSep = Nplot()                     #create histograms 
            ...
            if n1.iloc[i,0] == 'no_pair':
                ...
                if len(chunks) < 8 and XYZsep < 20:
                    ...
                    Nenvironment()                                                       
                    xc1,xc2,yc1,yc2,mc1,mc2,R,mcT,mcf = Nrotation(random.randint(0, 628)/100)  #get the x,y and mass density data                                                                                      
                    Stackhaloes,StackRhaloes,StackSep,StackMR,StackFil,FilMassMRTally,FilMassSepTally,FilMassMR,FilMassSep = Nplot()
                    Nran.append(i)
                    count +=1
                    print(end-start)
