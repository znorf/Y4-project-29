...
print(end-start) 
n0,n1,pid,n0Chunks,n1Chunks,dataH13,rand,Stack,bins,StackRad,binss,Stackhaloes,StackSep,binsSep,binsMR,StackMR,StackValues,FilHaloes,SepTotal,MRTotal,select,SepTotalNorm,MRTotalNorm,StackRVir1,StackRVir2= load(5)
...
for i in rand:  
    ...
    if n0.iloc[i,0] == 'no_pair':
        ...
        if len(chunks) < 8 and XYZsep < 20:                                     #ignoring wierd chunk boundary plots and (currently) ignoring pairs that go over due to periodicity
            ...
            Nenvironment()                                                       
            xc1,xc2,yc1,yc2,mc1,mc2,R,mcT,mTot = Nrotation(random.randint(0, 628)/100)  #get the x,y and mass density data                                                                                      
