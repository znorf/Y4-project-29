...
def load(select):
    ...
    StackRad = np.zeros(len(binss)-1) 
    StackHalo,StackSep,StackMR,FilHaloes,StackVal,StackRVir1,StackRVir2 = [],[],[],[],[],[],[]
    ...
    SepTotalNorm = np.zeros([select+1,spaces-1,])
    spaces = 10
    binsMR = np.linspace(0,1,spaces)
    MRTotal = np.zeros([select+1,spaces-1]) 
    MRTotalNorm= np.zeros([select+1,spaces-1]) 
    return Ppairs1,Ppairs2,Ppairs11,Ppairs22,df_Chunk,rand,Stack,bins,select,StackRad,binss,StackHalo,StackSep,binsSep,binsMR,StackMR,StackVal,FilHaloes,SepTotal,MRTotal,SepTotalNorm,MRTotalNorm,StackRVir1,StackRVir2
...
def Dataset():
    ...
    if ran == 0:                        
        ...
        R2 = df1.iloc[int(Ppairs1.iloc[i,0]),2]/1000
        ...
        if Ppairs11.iloc[i,0]== Ppairs22.iloc[i,0]:
            ...
            R1 = df1.iloc[int(Ppairs2.iloc[i,0]),2]/1000 
...
def Protation(theta_z,theta_y):                           #A function for physical pairs to create arrays containing x and y coordinates of DM haloes and their mass densities. This is done for the two clusters separately. Cluster 2 is rotated by a random value between 0 and 2 pi, given by the variable 'rotate'. 
    global rot
    xc1,yc1,mc1,mTot = [],[],[],[]                       #reset all the arrays
    mcT,R = [],[]
...
def Pplot():                                                                                    #plotting sum of physical and non-physical pairs    
    ...
    for i in range(len(binsSep)):
        if XYZsep < binsSep[i]:
            SepTotal[count,i-1] = sum(mTot)
            SepTotalNorm[count,i-1] = sum(mTot)/XYZsep
            break
    for i in range(len(binsMR)):
        if MR < binsMR[i]:
            MRTotal[count,i-1] = sum(mTot)
            MRTotalNorm[count,i-1] = sum(mTot)/XYZsep
            break
...
def Final():
    ...
    plt.xlim(3,20)
    plt.savefig('PSep.png')
    np.savetxt("Pmidss_ds.csv", midss, delimiter=",")
    np.savetxt("Px_val_ds.csv", x_val, delimiter=",")
    np.savetxt("Px_count_ds.csv", x_count, delimiter=",")
    np.savetxt("Perrors_x_ds.csv", errors_x, delimiter=",")
    np.savetxt("Perrors_y_ds.csv", errors_y, delimiter=",")
    
    #M_stack/ds vs Separation (ds)
    ...
    plt.xlim(3,20)
    plt.savefig('PSepNorm.png')
    np.savetxt("Pmidss_ds2.csv", midss, delimiter=",")
    np.savetxt("Px_val_ds2.csv", x_val, delimiter=",")
    np.savetxt("Px_count_ds2.csv", x_count, delimiter=",")
    np.savetxt("Perrors_x_ds2.csv", errors_x, delimiter=",")
    np.savetxt("Perrors_y_ds2.csv", errors_y, delimiter=",")
    ...
    plt.xlim(0,1)
    plt.savefig('PMR.png')
    np.savetxt("Pmidss_ds3.csv", midss, delimiter=",")
    np.savetxt("Px_val_ds3.csv", x_val, delimiter=",")
    np.savetxt("Px_count_ds3.csv", x_count, delimiter=",")
    np.savetxt("Perrors_x_ds3.csv", errors_x, delimiter=",")
    np.savetxt("Perrors_y_ds3.csv", errors_y, delimiter=",")
    ...
    plt.errorbar(midss, x_val/x_count, xerr=errors_x, yerr=errors_y, fmt='-o', capsize=5, label='Data ± Error')  #gives errors if divide by 0
    plt.xlabel('Mass Ratio')
    plt.ylabel('(Total Filament Mass/ Separation distance)/solar masses')
    plt.xlim(0,1)
    plt.savefig('PMRNorm.png')
    np.savetxt("Pmidss_ds4.csv", midss, delimiter=",")
    np.savetxt("Px_val_ds4.csv", x_val, delimiter=",")
    np.savetxt("Px_count_ds4.csv", x_count, delimiter=",")
    np.savetxt("Perrors_x_ds4.csv", errors_x, delimiter=",")
    np.savetxt("Perrors_y_ds4.csv", errors_y, delimiter=",")
...
Ppairs1,Ppairs2,Ppairs11,Ppairs22,df_Chunk,rand,Stack,bins,select,StackRad,binss,StackHalo,StackSep,binsSep,binsMR,StackMR,StackVal,FilHaloes,SepTotal,MRTotal,SepTotalNorm,MRTotalNorm,StackRVir1,StackRVir2 = load(200)
...
for i in rand:  
    ...
    if XYZsep < 20:         #for now ignoring plots where cluster 1 and cluster 2 are on the opposite sides of the box due but are pairs due to periodicity. 
        ...
        Penvironment()                                                       
        xc1,yc1,mc1,R,mcT,mTot = Protation(theta_z,theta_y)  #get the x,y and mass density data                                                                                     
...
#Instructions to use:
#Make sure that you have installed all of the: Chunk parquets (64 total),  'Physical_Pair_Chunk_i_0.parquet' 'Physical_Pair_Chunk_i_1.parquet',
#'Physical_Pair_chunk_0.parquet', 'Physical_Pair_chunk_1.parquet','P_Chunks.parquet' 
#This code must be ran after the non-physical pair code. This is because the non-physical pair code selects the pair haloes and saves them in the file "Nran.csv".
